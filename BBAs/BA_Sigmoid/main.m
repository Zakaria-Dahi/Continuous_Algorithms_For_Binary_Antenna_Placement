...
%% Prameter  of the Bat algorithm  ----------------------
     ...
     segma  = 0.2;               % Paremeter extracted from the article :  BCS:A BINARY CUCKOO SEARCH ALGORITHM FOR FEATURE SELECTION
...
%% variable used during the calcul ----------------------------------------
     ...
     %%%% The population 
     population = [];
     population_bin = ones(indiv,Dimension);
...
RANDOM_POPULATION = rand(indiv,Dimension);
% --------------------------------------------------------------------
%%--------------- Generate the binary solution ----------------------
for w=1:indiv
    population_bin(w,:) = ACTIVATE(population(w,:),RANDOM_POPULATION(w,:),Dimension);
end
...
gbest_rand = RANDOM_POPULATION(indbest,:);
...
%% ------------ Generate the initial random matrix -----------------
RANDOM_POPULATION = rand(indiv,Dimension);
RANDOM_POPULATION(indbest,:) = gbest_rand;
% --------------------------------------------------------------------
for  w=1:indiv% ------------ loop of flower processing  global or local pollination ------
         ...
         % -----------------------------------------------------------------
         %% ----------- Activation function sigmoid , hyperbolique ect -----------------------
             population_bin(w,:) = ACTIVATE (population(w,:),RANDOM_POPULATION(w,:),Dimension);
...
GBEST_ALL = ACTIVATE(gbest,gbest_rand,Dimension);
end
