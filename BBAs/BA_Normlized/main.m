...
%% variable used during the calcul ----------------------------------------
     ...
     %%%% save the population before normalisation 
     population_norm = [];
...
population_norm = GeneratePopulation(indiv,Dimension,minim,maxim);
% --------------Generate binary population ----------------------
for h=1:indiv
population(h,:) = norm_func(Dimension,population_norm(h,:),population(h,:));
end
...
for  w=1:indiv% ------------ loop of flower processing  global or local pollination ------
      %% ------- memorize the previous state of the population before global or local pollination ---------
              populationsave_norm = population_norm;
      ...
      % -----------------------------------------------------------------
          ...
          % ------The prosessing of the bat algorithm ----------------------
            % --------Generate new solutions ----------------------
               ...
               v(w,:)= v(w,:)+(population_norm(w,:) - gbest)*Q(w);
               population_norm(w,:) = population_norm(w,:) + v(w,:);   
             % -------- Do local search ----------------------
		   if rand>r(w)
               E = -1 + (1 -(-1))* rand;
               population_norm(w,:) = population_norm(w,:) + (E * (mean(A)));
           end
         % -----------------------------------------------------------------
         %% ---- Activation via normalisation ------------------------------------------
            population(w,:) = norm_func(Dimension,population_norm(w,:),population(w,:));
...
%% ----- If new solutions are better, update them in   the population ------
           if  (result < fit(w)) || (isequal(num2str(result),'NaN') == 1)
               population_norm = populationsave_norm;
               ...
               if (rand<A(w)) & (result < fitness_best)
                   fit(w) = result ;
                   A(w) = alpha*A(w);
...
GBEST_ALL = gbest;
GBEST_ALL = norm_func(Dimension,GBEST_ALL,GBEST_ALL);
end
