...
for w=1:indiv
        ...
        result  = Fitness(b_population,Dimension,radius_1,GRID_SIZE_X,GRID_SIZE_Y,SQUARE_XY,mask_1); %% c'est OK
        fit = [ fit , result];
        else
            if shape == 2
               result  = Fitness_two(b_population,Dimension,radius_2,GRID_SIZE_X,GRID_SIZE_Y,DIRECTIVE_XY,mask_2); %% c'est OK
               fit = [ fit , result];  
            end
            if shape == 3
               result  = Fitness_three(b_population,Dimension,radius_3,GRID_SIZE_X,GRID_SIZE_Y,DIRECTIVE_XY,mask_3); %% c'est OK
               fit = [ fit , result];
            end
            if shape == 123
               DECIMAL_VECT = DECIMAL(b_population,Dimension);
...
for  w=1:indiv% ------------ loop of flower processing  global or local pollination ------
 ...
 %% ---------------------- Evaluate the new solutions---------------------------
    if shape == 1 %% if you choose squared shape antenna
        ...
        if shape == 2
        result  = Fitness_two(b_population,Dimension,radius_2,GRID_SIZE_X,GRID_SIZE_Y,DIRECTIVE_XY,mask_2);
        end
        if shape == 3
        result  = Fitness_three(b_population,Dimension,radius_3,GRID_SIZE_X,GRID_SIZE_Y,DIRECTIVE_XY,mask_3); %% c'est OK
        end
