...
% Variable parameter of the Quantum Interference 
     LR = 0.10;     
...
%% variable used during the calcul ----------------------------------------
     ...
     populationsave = [];
     %%%% save randomly generated population 
     population_rand = rand(indiv,Dimension);
...
%-------------------------------------------------------------------
%% ------------ Quantum Activation ---------------------------------
population_bin =  rand(indiv,Dimension);
for i=1:indiv
    population_bin(i,:) = quant_act(population_bin(i,:),population(i,:),population_rand(i,:));
end
%-------------------------------------------------------------------
...
gbest_bin = population_bin(indbest,:);
...
for  w=1:indiv% ------------ loop of flower processing  global or local pollination ------
         %% ------------ Generate Random Population -------------------------
            population_rand = rand(indiv,Dimension);
         %-------------------------------------------------------------------   
         %% ----- save previous velocities and frequencies and loudnesses and amplitudes ----
              v_save = v;
              A_save = A;
              r_save = r;
              Q_save = Q;		 
         ...
         % -----------------------------------------------------------------
         %% ---------------- Acheiving Interference -----------------------------------------
             population(w,:) =  ((population(w,:) * LR) + gbest_bin)/ (1 + LR);
         %-----------------------------------------------------------------------------------
             population_bin(w,:) = quant_act(population_bin(w,:),population(w,:),population_rand(w,:));
         %-----------------------------------------------------------------------------------
